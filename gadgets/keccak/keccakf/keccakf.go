// Code modified from github.com/consensys/gnark/std/permutation/keccakf. Below is the
...
// Permute applys the keccakf permutation on the input state of 1600 bits, then outputs the permuted new state of 1600 bits
// the range check [0, 1] for each element in 'a' must be done outside already
func Permute(api frontend.API, a [1600]frontend.Variable) [1600]frontend.Variable {
	var in [25]xuint64
	for i := 0; i < 25; i++ {
		in[i] = asUint64(a[i*64 : i*64+64])
	}
	res := permute(api, in)
	var out [1600]frontend.Variable
	for i := range res {
		copy(out[i*64:i*64+64], fromUint64(res[i]))
	}
	return out
}
