...
func GetTransactionMptProofWitness() core.TransactionMptCircuit {
	...
	legacyKeccak256 := sha3.NewLegacyKeccak256()
	_, _ = legacyKeccak256.Write(leafBytes)
	leafHashBytes := legacyKeccak256.Sum(nil)
...
func GetTransactionProofWitness() core.TxHashCheckCircuit {
	...
	// tx rlp keccak
	data := hexutil.MustDecode("0x" + valueHexString)

	paddedWithRandomLength := utils.Slice2FVs(keccak.Bytes2BlockBits(keccak.Pad101Bytes(data)))
	log.Infof("Padded with random length %d", len(paddedWithRandomLength))

	hash, err := hexutil.Decode("0x0938663d48503e8e4b5c380c6b84ba3167e95b480b4693c374880b49b1cd1fd3")
	if err != nil {
		log.Fatal(err)
	}
	var txHashPiece [2]frontend.Variable
	txHashPiece[0] = hash[0:16]
	txHashPiece[1] = hash[16:32]
	...
	blockRlpRoundIndex := keccak.GetKeccakRoundIndex(len(blockRlpHex) - 2)
	...
	witness := core.TxHashCheckCircuit{
		...
		BlockFieldsNum:       17,
		BlockRoundIndex:      blockRlpRoundIndex,
	}
	return witness
}
